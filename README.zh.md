...
Jev 不生成文本：给定 `state` 与类型化问题，它用**一次并行 API 调用**返回**带校准概率的类型化判定**（官方宣称 ~70–500ms）。本插件把这一能力封装成两个 Agent 工具，并且坚持「验证过的才叫有效」：

| 工具 | 作用 |
| --- | --- |
| `jev_decision` | 对 `state` 一次性提出 Choice / Score / Noul 问题。返回类型化答案、置信度、概率分布、token 用量、实测延迟与估算成本。 |
| `jev_verify` | 对**线上真实 API** 运行内置带标签基准（21 个用例 / 25 个问题），返回实测准确率、中位/p95 延迟、置信校准与成本——这是防欺骗的自我验证。 |

**诚实设计，绝不造假**：

- 未配置 `TYPESAFE_API_KEY` 时，两个工具都会用明确的报错说明如何配置，而不是返回伪造答案；
...
TypeSafe AI（创始人 Diogo Almeida，前 OpenAI、ChatGPT 研究方向）于 2026-09-15 发布 Jev，定位首个「System One」模型：输入非结构化状态，输出类型化概率判定。在智能体高频原子判定场景（分类、路由、分诊、评分、护栏、真伪判断）上，Jev 比前沿对话 LLM 快数十到两百倍（输入 $0.042/百万 token，输出免费，250k tokens/s，1200 req/min）。适合承担 Agent 循环里高频「快判断」，把「慢思考」留给大模型。
...
重启 profile（或用插件市场一键安装，市场会自动完成以上两步）。工具即出现在 Agent 的工具目录中。
...
随时可让 Agent 执行 `jev_verify` 确认端点健康并获取实测数据。
...
**当前状态**：✅ **已于 2026-09-21 对线上 API 实测**（`jev-latest`）：准确率 **96.0%**（72/75，三轮运行一致），中位延迟 **308 ms**（p95 949 ms），75 个并行问题全程成本 **≈ $0.001**。完整数据、唯一边界误标（severity 得分 0.01 vs 期望 0）及与官方口径对照表见 [docs/verification.md](docs/verification.md)。

随时可复现：

```sh
TYPESAFE_API_KEY=... node bench/bench.mjs            # 1 轮（25 个问题）
...
本插件的差异化：**完整干净的原语 + 诚实可复现的验证基准**，直面真实 TypeSafe API。
