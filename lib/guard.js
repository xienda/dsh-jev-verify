...
export function createGuardModule({ requestSystemOne }) {
  ...
  const SAFETY_PATTERNS = [
    ...
    { name: "credential/env exfiltration", re: /(cat|type|get-content|printenv|env)\s+.*\.env|aws\s+secretsmanager.*get-secret|\bchmod\s+777\b.*(ssh|pem|key)|token\s*=\s*["']?(ghp_|sk-)/i },
  ];
   ...
   */
  function applyAutoGuard(ctx, config, options) {
