...
export const Config = z
  ? z.object({
      enabled: z.boolean().default(true).description("总开关：关闭后不注册任何 Jev 工具，Jev 模型不参与会话（护栏与仪表盘也随之停用）"),
      ...
      dashboard: z
        .object({
          enabled: z.boolean().default(true).description("本机 Jev 可视化仪表盘（web profile 的 /jev 页面）"),
          basePath: z.string().default("/jev").description("仪表盘挂载路径"),
        })
        .default(() => ({})),
    })
  : null;
  ...
  const guardExtra = () =>
    guardRuntime
      ? {
          ...
          budgetRemaining: Math.max(0, (config.autoGuard?.maxJevCallsPerSession ?? 50) - guardRuntime.gs.safety.jevCalls - guardRuntime.gs.loop.jevCalls),
        }
      : {};
  try {
    ctx.inject(["webServer"], (webCtx) => {
      if (!globalThis.__dshJevDashboardMounted) {
        globalThis.__dshJevDashboardMounted = true;
        dashboard().registerRoutes(webCtx, config, opts, guardExtra);
      }
    });
  } catch {
    /* host without webServer (headless etc.): no dashboard, tools still work */
  }
