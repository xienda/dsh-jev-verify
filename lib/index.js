...
export const Config = z
  ? z.object({
      ...
      dashboard: z
        .object({
          enabled: z.boolean().default(false).description("另有 /jev 网页版看板（默认关闭）；对话内随时用 jev_overview 查看"),
...
function apply(ctx, config) {
  ...
  try {
    if (liveConfigOf().dashboard?.enabled === true) {
      ...
      // keep the roll warm so jev_overview works without the page
      dashboard().record; // touch (no-op) - roll is always active via record()
    }
  } catch {
